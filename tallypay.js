...
window.onload=function(){
	const ss = document.getElementsByTagName('link'); var ss_count = 0;	
	for (let i = 0; i < ss.length; i++) { var count = count + (ss[i].getAttribute('href').match(/tallypay/g) || []).length; }
	if(ss_count == 0){
	...
	if(typeof tc_logged_in_as !== 'undefined'){ var logged_in_as = tc_logged_in_as;  }else{ var logged_in_as = ''; }
...
function init_tallypay_widget(logged_in_as){
	...
	Array.prototype.forEach.call(widget, function(el) {
			...
			if(el.dataset.ex_rates == 'exclude'){ var e = "N"; }else{ var e = "Y"; }
			var theme = "light"; if(typeof el.dataset.theme !== 'undefined' && (el.dataset.theme == "light" || el.dataset.theme == "dark")){ var theme = el.dataset.theme; }			
			var button_text = ''; var clickable_image = ''; var success_callback = '';
			if(el.dataset.size == 'button'){ 
				if(el.dataset.button_text != ''){ var button_text = el.dataset.button_text; }
				var s = "Y";
			}else{ var s = "N"; }
			if(typeof el.dataset.image !== 'undefined'){ var clickable_image = el.dataset.image; }
			if(typeof el.dataset.onpayment !== 'undefined'){ var success_callback = el.dataset.onpayment; }
			
			this[htmlid] = new tallypay(el.dataset.user,el.dataset.fundid,e,s,button_text,clickable_image,htmlid,theme,success_callback,logged_in_as);
	});		
}
	
var tallypay=function(user,fundid,ex_rates_inc,is_button,button_text,clickable_image,htmlid,theme,success_callback,logged_in_as){
	
	var fundtimer, paytimer, tc_spinner; var paychk = ""; var conw = ""; var conc = ""; var amount_type = 'Preset';
	var minw = 400; var maxw = 600; var maxh = 570; var minh = 470;
	if(is_button == "Y"){ var minw = 300; }
	
	addHTMLtoBody();
	
	if(logged_in_as == ''){ document.getElementById('public_chkbox-'+htmlid).checked = true; }
	...
	function call_api(params,endpoint){
		...
		if(endpoint == '/v1/fundraiser/index.php' || endpoint == '/v1/user/default_fundraiser/index.php'){ xhr.onload = function() {	
			if(this.response.hasOwnProperty('error') || this.response == 'null'){ tc_error_page(this.response.error); }else{ doProfile( this.response ); }
		} }	
	...
	function addHTMLtoBody(){
		...
		if(is_button == "Y" || clickable_image != ''){
			...
			if(clickable_image != ''){ 
				...
				btn += '	<div style="display:inline-block;cursor:pointer;"><img src="'+clickable_image+'"></div>';
			}else{
				btn += '<div id="tc_tip_button-'+htmlid+'" class="tc_tip_button">';
				btn += '	<div style="display:inline-block">'+button_text+'</div>';
			}
			btn += '	</div>';
						
			html += '<div id="tc_overlay-'+htmlid+'" class="tc_overlay_bg">';
			html += '	<div id="tc_cls_overlay-'+htmlid+'" class="tc_close_overlay">&times;</div>';
		...
		html += '	<div class="tc_padfive tc_bbox tc_loading theme_'+theme+'" id="tc_spinner-'+htmlid+'"><div class="tc_spinner rotate"></div><div style="font-size:1.5rem;font-weight:600;">please wait</div></div>';
			
		html += '	<div id="tc_front-'+htmlid+'" class="theme_'+theme+' tc_padfive tc_bbox tc_bbox-'+htmlid+'">	';
						
		html += '		<div class="tc_hidden theme_'+theme+'" id="tc_loaded-'+htmlid+'" style="display:none">';
		
		html += '			<div id="tc_keypad_panel-'+htmlid+'">';
		
		html += '				<div class="tc_pr_title_box">';
		html += '					<div class="tc_profile_img tc_centered" id="tc_profile_img-'+htmlid+'"></div>';
		html += '				</div>';
		
		html += '				<div class="tc_name_box">';
		html += '					<div class="tc_send_bitcoin_to">Send Bitcoin to</div>';
		html += '					<div class="tc_profile_name" id="tc_profile_name-'+htmlid+'"></div>';
		html += '				</div>';
	

		html += '				<div class="tc_keypad_wrapper">';			
		html += '					<div id="tc_amount_type" class="tc_centered">';
		html += '						<div class="tc_dropdown_txt" id="tc_dropdown_txt-'+htmlid+'">Preset Amount</div>';
		html += '						<div class="tc_right_float"><img src="https://tallyco.in/img/drop-down-arrow.svg" style="width:36px;height:36px;"></div>';
		html += '						<div style="clear:both"></div>';
		html += '					</div>';
		
		html += '					<div id="tc_amount_type_options">';
		html += '						<div id="tc_amount_option_preset-'+htmlid+'" class="tc_amount_option"><img src="https://tallyco.in/img/bitcoin-bag.svg" style="width:30px;height:30px;"> Preset Amount</div>';	
		html += '						<div id="tc_amount_option_item-'+htmlid+'" class="tc_amount_option"><img src="https://tallyco.in/img/coffee.svg" style="width:30px;height:30px;"> Buy Coffee / Dinner</div>';	
		html += '						<div id="tc_amount_option_custom-'+htmlid+'" class="tc_amount_option" style="border-bottom-left-radius: 5px;border-bottom-right-radius: 5px;"><img src="https://tallyco.in/img/calculator.svg" style="width:30px;height:30px;"> Customize Amount</div>';			
		html += '					</div>';	
		html += '				</div>';
		...
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_br tc_ps-'+htmlid+'" id="tc_pkeypad_1000-'+htmlid+'"><span class="tc_sym">$</span>10</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_b tc_ps-'+htmlid+'" id="tc_pkeypad_1500-'+htmlid+'"><span class="tc_sym">$</span>15</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_br tc_ps-'+htmlid+'" id="tc_pkeypad_2000-'+htmlid+'"><span class="tc_sym">$</span>20</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_br tc_ps-'+htmlid+'" id="tc_pkeypad_3000-'+htmlid+'"><span class="tc_sym">$</span>30</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_b tc_ps-'+htmlid+'" id="tc_pkeypad_4000-'+htmlid+'"><span class="tc_sym">$</span>40</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_r tc_cell_rbl tc_ps-'+htmlid+'" id="tc_pkeypad_5000-'+htmlid+'"><span class="tc_sym">$</span>50</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_r tc_ps-'+htmlid+'" id="tc_pkeypad_10000-'+htmlid+'"><span class="tc_sym">$</span>100</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_rbr tc_ps-'+htmlid+'" id="tc_pkeypad_20000-'+htmlid+'"><span class="tc_sym">$</span>200</div>';
		html += '				</div>';

		html += '				<div id="tc_item_keypad-'+htmlid+'" class="tc_keypad_grid tc_centered" style="display:none">';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_b tc_cell_rtl tc_cell_rtr tc_itm-'+htmlid+'" id="tc_ikeypad_300-'+htmlid+'" style="width: 100%;">Coffee <span class="tc_sym">($3)</span></div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_b tc_itm-'+htmlid+'" id="tc_ikeypad_1500-'+htmlid+'" style="width: 100%;">Lunch <span class="tc_sym">($15)</span></div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_b tc_itm-'+htmlid+'" id="tc_ikeypad_5000-'+htmlid+'" style="width: 100%;">Dinner <span class="tc_sym">($50)</span></div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_b tc_cell_rbl tc_cell_rbr tc_itm-'+htmlid+'" id="tc_ikeypad_10000-'+htmlid+'" style="border-bottom: 1px solid #e4e3f9;width: 100%;">Gift <span class="tc_sym">($100)</span></div>';
		html += '				</div>';
		
		html += '				<div id="tc_custom_keypad-'+htmlid+'" class="tc_keypad_grid tc_centered" style="display:none">';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_br tc_cell_rtl tc_kp-'+htmlid+'" id="tc_keypad_1-'+htmlid+'">1</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_br tc_kp-'+htmlid+'" id="tc_keypad_2-'+htmlid+'">2</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_b tc_cell_rtr tc_kp-'+htmlid+'" id="tc_keypad_3-'+htmlid+'">3</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_br tc_kp-'+htmlid+'" id="tc_keypad_4-'+htmlid+'">4</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_br tc_kp-'+htmlid+'" id="tc_keypad_5-'+htmlid+'">5</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_b tc_kp-'+htmlid+'" id="tc_keypad_6-'+htmlid+'">6</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_br tc_kp-'+htmlid+'" id="tc_keypad_7-'+htmlid+'">7</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_br tc_kp-'+htmlid+'" id="tc_keypad_8-'+htmlid+'">8</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_b tc_kp-'+htmlid+'" id="tc_keypad_9-'+htmlid+'">9</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_r tc_cell_rbl tc_kp-'+htmlid+'" id="tc_keypad_dot-'+htmlid+'">.</div>';
		html += '					<div class="tc_keypad_cell tc_keypad_cell_'+theme+' tc_cell_r tc_kp-'+htmlid+'" id="tc_keypad_0-'+htmlid+'">0</div>';
		...
		var la = '';
		if(logged_in_as == '' && (window.location.hostname == 'localhost' || window.location.hostname == 'tallyco.in' || window.location.hostname == 'tallycoin.app')){ var la = '<span class="loginlink" onclick="navLoginOptions()">or login</span>'; }
		if(logged_in_as == ''){ var ch = 'checked disabled'; }else{ var ch = ''; }
		
		html += '				<div class="tc_block">';
		html += '					<div class="tc_message_box"><textarea id="tc_message-'+htmlid+'" type="text" placeholder="Type a Public Message (optional) ..." style="resize: none;" maxlength="140"></textarea></div>';		
		html += '					<div class="tc_block">';
		html += '						<div class="tc_chkbox" style="float:left;position:relative;"> <input id="public_chkbox-'+htmlid+'" name="public_chkbox-'+htmlid+'" type="checkbox" '+ch+'><span class="tc_checkmark"></span> </div> <div class="tc_tag_me"><label for="public_chkbox-'+htmlid+'" class="noselect">anonymous</label>'+la+'</div>';
		html += '						<div class="tc_char_count" id="tc_char_count-'+htmlid+'">140</div>';
		html += '						<div style="clear:both"></div>';
		html += '					</div>';
		html += '				</div>';
		
		html += '				<div class="tc_pay_method_boxes">';
		html += '					<div class="tc_don_btn_wr"><div id="tc_btc_don_btn-'+htmlid+'" class="tc_don_click-'+htmlid+' tc_donate_btn tc_btn"><img src="https://tallyco.in/img/chain.svg"><span>on-chain</span></div></div>';
		html += '					<div class="tc_don_btn_wr" style="margin-left: 1.5%;" id="tc_ln_b_w-'+htmlid+'"><div id="tc_ln_don_btn-'+htmlid+'" class="tc_don_click-'+htmlid+' tc_donate_btn tc_btn"><img src="https://tallyco.in/img/lightning.svg"><span>lightning</span></div></div>';
		html += '					<div style="clear:both"></div>';
		html += '				</div>';
		...
		html += '	<div id="tc_back-'+htmlid+'" class="tc_bbox tc_bbox-'+htmlid+' theme_'+theme+'" style="display:none">';
				
		html += '		<div class="tc_hidden theme_'+theme+'" id="tc_pay_page-'+htmlid+'">';
		html += '			<div class="tc_padfive">';
		html += '			<div class="tc_flz"><a id="tc_pay_back-'+htmlid+'" class="tc_link"><div class="tc_btn_txt tc_font_bold"><img src="https://tallyco.in/img/left-arrow-light.svg" style="width:10px;height:10px;">  back</div></a></div>';
		html += '			<div class="tc_flz"><a id="tc_paid_back-'+htmlid+'" class="tc_link"><div class="tc_btn_txt tc_font_bold"><img src="https://tallyco.in/img/left-arrow-light.svg" style="width:10px;height:10px;">  back</div></a></div>';
		html += '			<div style="display:inline-block;float:right;"><a id="tc_pay_back-'+htmlid+'" class="tc_link"><div class="tc_btn_txt tc_font_bold tc_pay_timer" id="tc_pay_timer-'+htmlid+'"></div></a></div>';
		html += '			<div style="clear:both;margin-bottom:5px;"></div>';

		html += '			<div class="tc_block" id="prev_message_wr-'+htmlid+'" style="text-align:center;">';
		html += '				<div class="tc_profile_name" style="margin-bottom:40px;">Preview</div>';
		html += '				<div class="tc_block">';
		html += '					<div class="tc_message_avatar" id="prev_message_avatar-'+htmlid+'"></div>';
		html += '					<div class="tc_message" id="prev_message_txt-'+htmlid+'"></div>';
		html += '				</div>';
		html += '			</div>';
			
		html += '			<div class="tc_wide_txt_c" id="qr_code_wr-'+htmlid+'">	';
		html += '				<div class="tc_profile_name" style="margin:40px 0 40px 0;">Scan to Pay</div>';
		html += '				<div id="tc_qr-'+htmlid+'" class="tc_qr_code"><div class="tc_copied" id="tc_copied-'+htmlid+'">COPIED</div></div>';
		html += '				<div style="display:block;text-align:center;"><div class="tc_send_exact" id="tc_exact-'+htmlid+'"></div></div>';
	...
	function insertHTML(){		
		...
		if(is_button == "Y" || clickable_image != ''){ 
			...
			document.getElementById('tc_tip_button-'+htmlid).style.display = 'inline-block';
		}
		if(user != "error"){ document.getElementById('tc_error_panel-'+htmlid).style.display = 'none'; }
		
		var b = document.getElementById('tc_panel_'+htmlid).getBoundingClientRect();
		
		if(is_button != "Y"){	
			var c = document.getElementsByClassName('tc_bbox');		
			Array.prototype.forEach.call(c, function(el) {	
				if(b.height == 0 || clickable_image != ''){ var h = 950; }else{ var h = b.height + 20; }
				document.getElementById(el.id).style.height = h+'px'; 
				if(b.width == 0){ var w = 400; }else{ var w = b.width; }
	...
	function populateCurrencies(exchange_rates){
		var tc_tc_ex_rates = document.getElementById('tc_currency_list-'+htmlid);
		
		for(var ticker in exchange_rates) { 
			var option = document.createElement("option");
			option.text = ticker; option.value = ticker;
			if(ticker == 'BTC'){ option.selected = 'selected'; }
			tc_tc_ex_rates.appendChild(option);
		}	
	}
	
	function trunc_string(string,max_length){
		if(string.length > max_length){ var fixed_length = max_length; var e = '...'; }else{ var fixed_length = string.length; var e = ''; }
	...
	function clickables(fown_handle,fown_name){	
		...
		Array.prototype.forEach.call(c, function(el) {	document.getElementById(el.id).onclick = function() { tc_keypad_preset(el.id,'i'); }	});	
		var c = document.getElementsByClassName('tc_ps-'+htmlid);
		Array.prototype.forEach.call(c, function(el) {	document.getElementById(el.id).onclick = function() { tc_keypad_preset(el.id,'p'); }	});		
		var c = document.getElementsByClassName('tc_kp-'+htmlid);
		Array.prototype.forEach.call(c, function(el) {	document.getElementById(el.id).onclick = function() { tc_keypad_press(el.id); }	});	
		var c = document.getElementsByClassName('tc_don_click-'+htmlid);
		Array.prototype.forEach.call(c, function(el) {	document.getElementById(el.id).onclick = function() { tc_donate_amount('show'); }	});			
		
		document.getElementById("tc_amount_type").onclick = function() { tc_amount_type_toggle(); }	
		document.getElementById("tc_amount_option_custom-"+htmlid).onclick = function() { tc_select_amount_type('Custom'); }
		document.getElementById("tc_amount_option_preset-"+htmlid).onclick = function() { tc_select_amount_type('Preset'); }
		...
		if(is_button == "Y"  || clickable_image != ''){ 
			...
			document.getElementById('tc_cls_overlay-'+htmlid).onclick = function() { tc_overlay_close(); }
		}
	}
	
	function tc_select_amount_type(type){
		amount_type = type;
		if(type == 'Item'){ var t = 'Buy Coffee / Dinner'; }
		if(type == 'Preset'){ var t = 'Preset Amounts'; }
		if(type == 'Custom'){ var t = 'Customize Amount'; }
		
		document.getElementById('tc_dropdown_txt-'+htmlid).innerHTML = t;
		document.getElementById('tc_preset_keypad-'+htmlid).style.display = 'none';
		document.getElementById('tc_custom_keypad-'+htmlid).style.display = 'none';
		document.getElementById('tc_item_keypad-'+htmlid).style.display = 'none';
		
		if(type == 'Custom'){
			document.getElementById('tc_custom_keypad-'+htmlid).style.display = 'flex';
		}
		if(type == 'Preset'){
			document.getElementById('tc_preset_keypad-'+htmlid).style.display = 'flex';
		}
		if(type == 'Item'){
			document.getElementById('tc_item_keypad-'+htmlid).style.display = 'flex';
		}
	}
	
	function tc_amount_type_toggle(){
		if(document.getElementById('tc_amount_type_options').style.display == 'block'){
			document.removeEventListener('click', outsideClickListener, true);
			document.getElementById('tc_amount_type_options').style.display = 'none';
			document.getElementById('tc_amount_type').style.background = 'white';
			document.getElementById('tc_amount_type').style.borderColor = '#e4e3f9';
		}else{
			document.addEventListener('click', outsideClickListener, true);
			var b = document.getElementById('tc_amount_type').getBoundingClientRect();
			document.getElementById('tc_amount_type_options').style.display = 'block';
			document.getElementById('tc_amount_type_options').style.width = b.width+'px';
			document.getElementById('tc_amount_type').style.background = '#f5f5f5';
			document.getElementById('tc_amount_type').style.borderColor = 'black';
		}	
	}
	...
	function tc_btn_clicked(){
		document.getElementById('tc_overlay_wrapper_'+htmlid).style.display = 'inherit';
	...
	function doProfile(fdata){
		if(tc_spinner != 'OFF'){ 
			insertHTML(); 
			clickables(fdata.fundraiser_owner_handle,fdata.fundraiser_owner_name);  
			if(fdata.lightning_available == 'N'){ document.getElementById('tc_ln_don_btn-'+htmlid).style.display = 'none'; }
			populateCurrencies(fdata.exchange_rate);
			tc_ex_rates = fdata.exchange_rate;
		}	
		if(fundid === undefined){ var l = fdata.fundraiser_url; fundid = l.replace("https://tallyco.in/s/",""); }
		
		var profile_url = 'https://tallyco.in/'+fdata.fundraiser_owner_handle+'/';
		document.getElementById('tc_profile_img-'+htmlid).setAttribute("style","background:url('"+fdata.fundraiser_owner_avatar+"');background-size:120px 120px;cursor:pointer;");		
		document.getElementById('tc_profile_img-'+htmlid).onclick = function(){ window.open(profile_url,"_self"); }
		document.getElementById('tc_profile_name-'+htmlid).innerHTML = trunc_string(fdata.fundraiser_owner_name,31);
	}
	...
	function prettyTime(time){
		...
		seconds  -= mnts*60;
		if(days > 0){ return days+' days '+hrs+' hrs '+mnts+' mins'; }else{ return hrs+' hrs '+mnts+' mins '+seconds+' secs'; }
	}
	...
	function tc_char_count(htmlid){
		...
		if(remaining < 30){ var c = '#f29a9a'; }else{ var c = "#e3e3e3"; }
		document.getElementById('tc_char_count-'+htmlid).style.backgroundColor = c;
	}
	...
	function tc_keypad_press(id){
		...
		if(num == 'dot'){ num = "."; } if(num == 'back'){ v = v.toString().substring(0, v.length - 1); num = ''; }
		document.getElementById("tc_donate_amount_val-"+htmlid).value = v + num;
		tc_denom();
	}
	...
	function tc_keypad_preset(id, i){
		...
		var v = Math.ceil((cents/100)*100000000/parseFloat(tc_ex_rates['USD'].rate));
		document.getElementById("tc_donate_amount_val-"+htmlid).value = v;
		tc_denom();
	}
	
	function tc_denom(){
		var donation_amount = document.getElementById('tc_donate_amount_val-'+htmlid).value; 
		if(donation_amount == ""){ donation_amount = ''; } donation_amount = donation_amount.toString().replace(/[^0-9.]*/g, '');
		donation_amount = donation_amount.replace(/./g, function(a, n, str) { if(a == '.'){ return str.indexOf(a) == n ? a : ""; }else{ return str.indexOf(a) == n ? a : a; }  });
		document.getElementById('tc_donate_amount_val-'+htmlid).value = donation_amount; 
		
		var fiat_value = tc_approx_fiat_value();
		if(fiat_value[0] == 'NaN'){ fiat_value[0] = 0; }
		if(fiat_value[1] != 'SAT'){ document.getElementById('tc_fiat_value-'+htmlid).innerHTML = 'Approx: '+fiat_value[0]+' '+fiat_value[1]; }else{ document.getElementById('tc_fiat_value-'+htmlid).innerHTML = ''; }
	}

	function tc_approx_fiat_value(){
		var donation_amount = document.getElementById('tc_donate_amount_val-'+htmlid).value; 	
		var denom = document.getElementById('tc_currency_list-'+htmlid); var denom = denom.options[denom.selectedIndex].value;	
		if(denom == 'BTC'){ var conv = 1; var fixed = 8; }else{ var conv = 0.00000001; var fixed = 2; }	
		var v = parseFloat(donation_amount)*parseFloat(tc_ex_rates[denom].rate)*conv;
	...
	function hidePanels(){
		...
		Array.prototype.forEach.call(x, function(el) { 
			var div = el.id.split("-");
			if(div[1] == htmlid){ document.getElementById(el.id).style.display = 'none'; }
		});
	...
	function tc_get_pay_request(cur,fallback){
		...
		if(cur == "btc" && satoshi_amount < 1000){ skip = "Y"; alert('Please select Lightning for amounts smaller than 1000 satoshis.'); }	
		if(cur == "ln" && satoshi_amount > 4200000){ skip = "Y"; alert('Please select Bitcoin for amounts larger than 0.042 BTC.'); }

		if(skip == "N"){
			document.getElementById('tc_front-'+htmlid).style.display = 'none';
			document.getElementById('tc_spinner-'+htmlid).style.display = 'block';
			
			document.getElementById('tc_sat_pending-'+htmlid).value = satoshi_amount;
	...
	function tc_pay_page( response ){
		if(response != ""){
			...
			document.getElementById("tc_pay_back-"+htmlid).style.display = 'inline-block';
			document.getElementById("tc_paid_back-"+htmlid).style.display = 'none';
			document.getElementById('tc_spinner-'+htmlid).style.display = 'none';
			document.getElementById('tc_pay_page-'+htmlid).style.display = 'block';
			document.getElementById("tc_qr-"+htmlid).style.display = 'block';
			document.getElementById("tc_ow-"+htmlid).style.display = 'inline-block';
			document.getElementById("tc_pay_timer-"+htmlid).style.display = 'inline-block';
			document.getElementById("tc_exact-"+htmlid).style.display = 'inline-block';	
			...
			if(response.message != ""){ 
				...
				document.getElementById('prev_message_txt-'+htmlid).innerHTML = response.message;
				document.getElementById('prev_message_avatar-'+htmlid).setAttribute('style','background:url("'+response.payer_avatar+'") #ffffff;background-size:50px 50px;'); 		
			}else{ document.getElementById("prev_message_wr-"+htmlid).style.display = 'none';  }
			...
			if(response.lightning_pay_request !== undefined){
				...
				var type = 'btc';
				var wallet_link = 'bitcoin:'+response.btc_address+"?amount="+cost_in_btc;
				var exact_send = 'Send exactly '+cost_in_btc+' BTC';
			}	
			
			var qrcode = tc_createQRCode(type,qrdata,cost_in_btc); 
			document.getElementById('qr_code_wr-'+htmlid).style.display = 'block';
			document.getElementById('tc_qr-'+htmlid).setAttribute('style','background:url("'+qrcode+'") #ffffff;background-size:300px 300px;');
			document.getElementById("tc_open_wallet-"+htmlid).href = wallet_link;
	...
	function doPayTimer(){		
		var paytimeleft = parseInt(document.getElementById("tc_pay_timer-"+htmlid).innerHTML);
		if(paytimeleft == "1"){ 
			clearInterval(paytimer); 
			document.getElementById("tc_pay_timer-"+htmlid).innerHTML = 'NO PAYMENT';
			document.getElementById('qr_code_wr-'+htmlid).style.display = 'none';	
			document.getElementById("prev_message_wr-"+htmlid).style.display = 'none';
			document.getElementById("tc_no_pay_vid-"+htmlid).style.display = 'block';	
			document.getElementById("tc_no_pay_gif-"+htmlid).src = "https://tallyco.in/img/unpaid_alert.gif";			
		}else{ document.getElementById("tc_pay_timer-"+htmlid).innerHTML = parseInt(paytimeleft)-1; }
				
		if(parseInt(paytimeleft) % 5 === 0 && parseInt(paytimeleft) < 294 && paychk == ""){
			paychk = "V";
			var tc_payment_id = document.getElementById('tc_payment_id-'+htmlid).value;
			call_api('payment_id='+tc_payment_id,'/v1/payment/verify/index.php'); 
		}		
	}
	...
	function verifyPayment(response){
		...
		if(response.payment_state == 'paid'){ 
			...
			document.getElementById("qr_code_wr-"+htmlid).style.display = 'none';
			document.getElementById("prev_message_wr-"+htmlid).style.display = 'none';
			document.getElementById("tc_paid_button_text-"+htmlid).style.display = 'block';
			document.getElementById("tc_pay_timer-"+htmlid).style.display = 'none';			
			document.getElementById("tc_pay_back-"+htmlid).style.display = 'none';
			document.getElementById("tc_paid_back-"+htmlid).style.display = 'inline-block';
			document.getElementById("tc_message-"+htmlid).value = '';
			document.getElementById("tc_donate_amount_val-"+htmlid).value = '';
			document.getElementById("tc_paid_gif-"+htmlid).src = "https://tallyco.in/img/paid_tick.gif";
	...
	function tc_createQRCode(type,data,amount){
		if(type == 'btc'){
			var typeNumber = 6;
			if(amount != ""){ var amount = "?amount="+amount; }	
...
function tc_copyToClipboard(htmlid){	
	...
	var elem = document.createElement('form'); elem.setAttribute('id' , 'tc_copy_form');
	var i = document.createElement("input"); i.setAttribute('type',"text"); i.setAttribute('value',tocopy); i.setAttribute('id','copy_this_to_clipboard');
	elem.appendChild(i); document.getElementById('tc_panel_'+htmlid).appendChild(elem);
	...
	if (document.body.createTextRange) {
		...
		else{ alert('Sorry. Copy on this device is not supported');	}
	}
	
	var element = document.getElementById('tc_copy_form'); element.parentNode.removeChild(element);
	setTimeout(function(){ document.getElementById('tc_copied-'+htmlid).style.display = 'none'; },1000);
